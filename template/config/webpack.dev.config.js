...
{{#if websocket}}
config.dev.proxyTable['ws://' + config.dev.host + ':' + config.dev.port + '/' + config.dev.targetApp + '/*'] = {
    target: 'ws://' + config.dev.targetHost + ':' + config.dev.targetPort,
    ws: true,
    secure: false,
    changeOrigin: true
};
    ...
    plugins: [
        ...
        new webpack.DefinePlugin({
            'process.env': {
                ...
                {{#if websocket}}
                WS_API: JSON.stringify("ws://" + config.dev.host + ":" + config.dev.port + "/" + config.dev.targetApp + "/"),
                {{/if}}
